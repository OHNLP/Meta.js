...
let fo = fs.createReadStream('./testsets/test_result_RR.csv');
...
function start_test() {
describe('testing metajs metabin for RR functions', () => {
   ...
   for (let i = 0; i < oc_names.length; i++) {
       ...
       it('case ' + ocn + ', fixed RR should be ' + _tf2(gtrs[0]) + '(' +_tf2(gtrs[1]) + ',' + _tf2(gtrs[2]) + ')', (done) => {
        ...
        it('case ' + ocn + ', random RR should be ' + _tf2(gtrs[3]) + '(' +_tf2(gtrs[4]) + ',' + _tf2(gtrs[5]) + ')', (done) => {
            var rst = metajs.metabin(vals, {
                sm: 'RR'
            });
